---
title: "causalForest - log maize yields take 2"
author: "Jill Deines"
date: "2/1/2022"
output: 
  html_document:
...
**Goal: run and evaluate causal forest on maize fields in IN, IL, IA, OH, MO, MI for 2019 & 2020**
...
library(knitr)
opts_chunk$set(cache=FALSE, fig.path='../../figure/02.06_causalForest_maize_log2/')
```
...
dataFile <- 'pointSample_kaiyuV2_20220127_maize_latLonRemoved.csv'

# scratch folder for model rdata objects - not in project repo (large files)
...
** transform Y yields to log scale **
...
both <- data.frame(W_hat = W_hat,
                   type = paste0('original sample (',nrow(CoverCropYearsAll), ' obs)')) %>%
...
X_c1 <- overlap %>%
  dplyr::select(-c (W,  uniqueID, fips5, w_hat, keep, cropType,contains('norm'),
                    ...
                    state_name, state, ccFreq, nccpisoy, yield_tha,
...
# reconstruct subsets
X_norm <- CoverCropYearsAll %>%
  dplyr::select(c(contains('norm'), "nccpicorn",'drclasscd','rootznaws',
